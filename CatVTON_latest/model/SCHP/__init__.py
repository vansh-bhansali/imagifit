...
class SCHP:
    def __init__(self, ckpt_path, device):
        ...
        # AugmentCE2P's context-encoding module uses AdaptiveAvgPool2d at fixed
        # scales (1x1, 2x2, 3x3, 6x6); MPS's adaptive pooling requires the input
        # size to evenly divide the output size (CPU/CUDA don't have this
        # restriction), so it crashes on arbitrary input resolutions. Fall back
        # to CPU on Apple Silicon.
        ...
        self.model = networks.init_model('resnet101', num_classes=self.num_classes, pretrained=None).to(self.device)
